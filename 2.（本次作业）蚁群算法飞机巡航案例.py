...
if __name__ == "__main__":
    ...
    rounds = 50  # 轮数
    ant_number = 50  # 蚂蚁数
    alpha = 1  # =0时容易陷入局部最优解，信息素指数
    beta = 1   # =0时收敛过快，也无法最优，可见度指数
    rho = 0.9  # 消散系数
    ...
    # 将最优TSP路径调整成成从1开始到1结束的路径序列
    if optimal_policy[0] != 1:
        start1_index = optimal_policy.index(1)
        lst_new = optimal_policy[start1_index:]+optimal_policy[1:start1_index]
        lst_new.append(1)
        optimal_policy = lst_new.copy()
